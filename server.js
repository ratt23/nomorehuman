...
const { generatePdfFromData } = require('./generate');
...
app.post('/process-file', upload.single('file'), async (req, res) => {
    try {
        ...
        const pdfPath = finalXlsxPath.replace('.xlsx', '.pdf');
        await generatePdfFromData(allAcceptedRows, pdfPath);
        res.json({ 
            ok: true, message: 'File berhasil diproses!', 
            excel: `/output/${path.basename(finalXlsxPath)}`, 
            pdf: `/output/${path.basename(pdfPath)}`,
...
// BARU: Ekspor fungsi untuk menyalakan server
module.exports.startServer = () => {
    return new Promise((resolve) => {
        app.listen(PORT, () => {
            console.log(`🚀 GraphiWorks berjalan di http://localhost:${PORT}`);
            resolve();
        });
    });
};