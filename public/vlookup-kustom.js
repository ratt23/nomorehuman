function initVlookupKustom() {
    ...
    vlookupInspectBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(window.API_BASE_URL + '/vlookup-inspect', { method: 'POST', body: formData, signal: controller.signal });
    ...
    vlookupProcessBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(window.API_BASE_URL + '/vlookup-process', { method: 'POST', body: formData });
            const result = await response.json();
            if (!result.ok) throw new Error(result.error);
