function initTarifGenerator() {
    ...
    tarifFileInput.addEventListener('change', async (e) => {
        ...
        try {
            const response = await fetch(window.API_BASE_URL + '/inspect-file', { method: 'POST', body: formData, signal: controller.signal });
            ...
            if (err.name === 'AbortError') {
                tarifResDiv.innerHTML = `<div class="report error"><strong>Pemeriksaan Gagal:</strong> Proses memakan waktu terlalu lama (lebih dari 30 detik).</div>`;
            } else {
    ...
    tarifProcessBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(window.API_BASE_URL + '/process-file', { method: 'POST', body: formData });
            const result = await response.json();
            if (!result.ok) throw new Error(result.error);
    ...
    async function performDoubleCheck() {
        ...
        openModal('Double Check Harga', '<div class="loading" style="padding: 40px;"><div class="spinner"></div><p>Membandingkan harga...</p></div>');
        try {
            const excelResponse = await fetch(window.API_BASE_URL + window.lastProcessedFiles.processed.excelUrl);
            ...
            const response = await fetch(window.API_BASE_URL + '/double-check-tarif', { method: 'POST', body: formData });
            const result = await response.json();
            if (!result.ok) throw new Error(result.error);
